...
`UI component → feature store method → Command → GameEngine.execute(state, command) → new GameState + GameEngineEvent[] → the store reacts to the returned events into UI-only signals (rejections, toasts, modal state, map selection) → components re-render off signals.`
...
- `state/` is a set of small `@ngrx/signals` feature stores, one per gameplay concern, instead of one monolithic facade: `core/game-core.store.ts` (`GameCoreStore` — the authoritative `GameState` signal, derived computeds like `activePlayer`/`victoryStatus`/`projectedIncome`, and the **only** place `GameEngine.execute()` runs, via `dispatch(command): readonly GameEngineEvent[]`), `map/map-ui.store.ts` (`MapUiStore` — selection/hover/drag/pending-action + the 4 map animation-effect queues), `combat/combat.store.ts`, `movement/movement.store.ts`, `economy/economy.store.ts`, `cyber/cyber-attack.store.ts`, `ai/ai.store.ts` (Solo Command Mode-only state and bonus commands), and `solo-setup/solo-setup.store.ts`. Components inject only the store(s) that own what they need — there is no single facade anymore. Every store's action methods call `GameCoreStore.dispatch(command)` themselves and react to the *returned* events for their own concern (e.g. `MovementRejected` → `movementRejectionReason`); a store whose dispatch can affect another store's concern (e.g. any move/attack/hack that can trigger a map effect) explicitly forwards the same events to that other store's `reactToEvents(events)` — see `state/shared/rejection-event-routing.ts`'s `clearsAllRejectionBanners()` for the shared "clear stale rejection banners on any unrelated success" convention every store's `reactToEvents` follows. `AiTurnService` and `AdvancePhaseBarComponent` are the only two callers of `GameCoreStore.advancePhase()`/`endTurn()` (which can emit almost any event type), so both fan the result out to every store via the shared `state/shared/route-turn-cycle-events.ts` helper. Adding a new event type usually means adding a case to whichever one store's `reactToEvents`/handler owns that concern, plus that helper's forward list if it's map/combat/movement/economy/cyber-relevant.
...
- `ui/overlays/toast-host/` mirrors various stores' `*RejectionReason` / `*ResultMessage` signals (movement, economy, cyber attack, combat, phase advance) as global transient toasts. This exists because several panels only render their own inline copy of a rejection conditionally (e.g. only during a specific phase), which can silently swallow a rejection the player needs to see.
...
- Tests: exhaustive gameplay-rule coverage is engine-only (`engine/**/*.spec.ts`), built on shared fixtures in `engine/test-fixtures.ts` (`testState()`, `player()`, `region()`, `unitInstance()`, etc.) — this layer is untouched by the store split. Each `state/**/*.store.spec.ts` adds smoke-level coverage only (initial state, one happy path, 1-2 reject paths), using `TestBed.configureTestingModule` with `DataLoaderService`/`RandomService` stubbed out so `GameCoreStore.initialize()` can run against a fake `InitialGameData` fixture. No component/UI tests exist yet.
